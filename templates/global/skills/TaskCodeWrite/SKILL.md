...
재사용과 함수화는 plan의 설계 결정을 따르며 다음 기준을 적용한다.

- 동일하거나 의미상 같은 코드가 반복되면 기존 helper를 재사용하거나 함수로 추출한다.
- 특정 객체에 의존하지 않는 util 성격의 기능은 함수로 추출한다.
- 재사용을 위해 호출부와 무관한 객체나 데이터를 넘겨야 한다면 함수화하지 않고 응집도 높은 현재 위치에 둔다.
- 위 기준이 충돌하면 불필요한 객체나 데이터 전달을 피하는 현재 위치 유지 판단을 우선한다.
- 단기간 내 재사용이 거의 확실하다는 코드·요구사항 근거가 있을 때만 미래 사용처를 위한 함수화를 진행한다.
- 확장 가능성만 있는 경우에는 현재 필요한 구현을 유지하고 실제 재사용 시점에 함수화한다.
- plan에 함수화 결정이 없거나 구현 중 근거가 달라졌다면 영향받는 task를 진행하기 전에 판단, 근거, tradeoff를 설명하고 사용자에게 확인한다.

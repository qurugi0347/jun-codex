...
3. 아래 순서로 설명을 구성한다.
   ...
   - **퀴즈**: 변경의 동작, 원인, 계약, 예외 상황을 실제로 이해해야 풀 수 있는 중간 난도 객관식 문제 5개를 만든다. 질문과 모든 선택지는 먼저 보이고, 정답과 선택지별 이유만 사용자의 선택 또는 펼치기 후 공개한다.
...
5. 사용자가 출력 형식을 지정하지 않으면 HTML로 만든다.
   ...
   - **Notion**: 사용자가 명시적으로 요청한 경우에만 Notion 도구로 새 페이지를 만들고 URL을 반환한다. 질문과 선택지는 일반 블록으로 표시하고, 정답과 선택지별 설명은 별도 toggle 블록으로 구성한다.
...
- HTML 퀴즈는 질문과 모든 선택지를 처음부터 표시하고, 선택 즉시 정답·오답 피드백과 이유를 보여 주며 오프라인에서도 동작하게 만든다.
